...
export async function generateRegistroCertificatePDF(input: RegistroCertificateInput): Promise<string> {
  ...
  const verifyUrl = hash ? `https://cyberregistro.com/verify?mlb=${encodeURIComponent(input.mlbCode)}&ts=${encodeURIComponent(ts)}&h=${hash}` : (input.permalink || 'https://cyberregistro.com');
