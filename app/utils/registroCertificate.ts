...
// Substitui caracteres fora de WinAnsi (PDF StandardFonts) por equivalentes seguros
function sanitizeWinAnsi(input: string): string {
  if (!input) return '';
  return input
    .replace(/[\u2010\u2011\u2012\u2013\u2014\u2015]/g, '-') // vários hifens/travessões
...
function drawJustifiedLine(page: any, words: { text: string; width: number }[], x: number, y: number, maxWidth: number, font: any, size: number, color: any, justify: boolean) {
...
function drawJustifiedParagraph(page: any, text: string, x: number, topY: number, maxWidth: number, minY: number, font: any, size: number, color: any, lineGap = 3) {
...
async function embedRemoteImage(pdfDoc: PDFDocument, url: string): Promise<any | null> {
  try {
    ...
    try { return await pdfDoc.embedPng(bytes); } catch { try { return await pdfDoc.embedJpg(bytes); } catch { return null; } }
  } catch { return null; }
}
...
export async function generateRegistroCertificatePDF(input: RegistroCertificateInput): Promise<string> {
  // A4 portrait
  const WIDTH = 595; const HEIGHT = 842; const M = 30;
  const pdfDoc = await PDFDocument.create();
  const font = await pdfDoc.embedFont(StandardFonts.Helvetica);
  const fontBold = await pdfDoc.embedFont(StandardFonts.HelveticaBold);

  // Carregar logo
  const logoResponse = await fetch('/cyber.png');
  const logoBytes = await logoResponse.arrayBuffer();
  const logoImage = await pdfDoc.embedPng(new Uint8Array(logoBytes));

  const page = pdfDoc.addPage([WIDTH, HEIGHT]);
  // Paleta igual à imagem de referência - Azul escuro + Cyan
  const bg = rgb(0/255, 31/255, 63/255); // Azul navy escuro #001F3F
  const bgDark = rgb(0/255, 15/255, 40/255); // Azul ainda mais escuro
  const bgMedium = rgb(0/255, 45/255, 80/255); // Azul médio
  const primary = rgb(0/255, 217/255, 255/255); // Cyan vibrante #00D9FF
  const primarySoft = rgb(102/255, 229/255, 255/255); // Cyan claro
  const accent = rgb(0/255, 217/255, 255/255); // Cyan accent
  const panel = rgb(0/255, 40/255, 75/255); // Azul escuro painel
  const panelLight = rgb(10/255, 50/255, 90/255); // Azul médio painel
  const white = rgb(1, 1, 1);
  const gray = rgb(150/255, 200/255, 230/255); // Azul claro para texto
  const grayDark = rgb(200/255, 230/255, 250/255); // Azul muito claro
  const grayLight = rgb(0/255, 80/255, 120/255); // Azul para bordas

  // Fundo base azul navy escuro
  page.drawRectangle({ x: 0, y: 0, width: WIDTH, height: HEIGHT, color: bg });

  // Gradiente sutil de azul (mais escuro no topo)
  page.drawRectangle({
    x: 0, y: HEIGHT - 200, width: WIDTH, height: 200,
    color: bgDark, opacity: 0.6
  });

  // Brilho azul no meio-inferior
  page.drawRectangle({
    x: 0, y: 200, width: WIDTH, height: 300,
    color: bgMedium, opacity: 0.3
  });

  // Círculos grandes com brilho cyan (decorativos)
  page.drawEllipse({
    x: WIDTH - 150, y: HEIGHT - 150, xScale: 180, yScale: 180,
    color: primary, opacity: 0.08
  });
  page.drawEllipse({
    x: WIDTH - 150, y: HEIGHT - 150, xScale: 150, yScale: 150,
    color: primary, opacity: 0.05
  });

  page.drawEllipse({
    x: -80, y: 200, xScale: 220, yScale: 220,
    color: primary, opacity: 0.06
  });

  page.drawEllipse({
    x: WIDTH / 2, y: HEIGHT / 2, xScale: 150, yScale: 150,
    color: primary, opacity: 0.04
  });

  // Círculos pequenos brilhantes espalhados
  const smallCircles = [
  ...
  smallCircles.forEach(circle => {
    page.drawEllipse({
      ...
      xScale: circle.size, yScale: circle.size,
      color: primary, opacity: 0.6
    });
  });

  // Linhas de conexão sutis (padrão de circuito)
  page.drawLine({
  ...
  // Selo © cyan brilhante
  const sealX = WIDTH - M - 58; const sealY = HEIGHT - M - 26;

  // Brilho cyan atrás
  page.drawEllipse({
    x: sealX + 28, y: sealY + 20, xScale: 30, yScale: 30,
    color: primary, opacity: 0.3
  });

  // Círculo azul escuro
  page.drawEllipse({
    x: sealX + 28, y: sealY + 20, xScale: 26, yScale: 26,
    color: bgDark
  });
  ...
  page.drawText('©', {
    x: sealX + 20, y: sealY + 8, size: 26,
    font: fontBold, color: white
  });

  // Título alinhado com a logo
  const titleX = M + logoSize + 20;
  const titleBaseY = logoY + (logoSize / 2) + 5; // Centro vertical da logo
  page.drawText('CERTIFICADO DE REGISTRO', {
    x: titleX, y: titleBaseY, size: 20,
    font: fontBold, color: white
  });
  ...
  // Cartão principal - Azul escuro com bordas cyan (igual à imagem)
  const cardX = M, cardY = HEIGHT - 155, cardW = WIDTH - M*2, cardH = 260;

  // Brilho cyan ao redor do cartão
  page.drawRectangle({
    x: cardX - 2,
    y: cardY - cardH - 2,
    width: cardW + 4,
  ...
  // Borda cyan brilhante principal
  page.drawRectangle({
    x: cardX,
    y: cardY - cardH,
    width: cardW,
  ...
  // Cantos decorativos cyan (estilo da imagem)
  ...
  // Autor da Obra (com CPF ao lado)
  page.drawText('Autor da Obra', { x: cardX + 15, y, size: 10, font: fontBold, color: white });
  ...
  if (autorCpf) {
    ...
    page.drawText(cpfLabel, { x: cardX + cardW - cpfWidth - 15, y, size: 10, font: fontBold, color: white });
  }
  y -= 15;
  const autorNomeValue = sanitizeWinAnsi(safe(input.autorNome) || safe(input.usuario?.nome) || `${safe(input.account?.firstName)} ${safe(input.account?.lastName)}`.trim() || safe(input.account?.nickname) || '-');
  page.drawText(autorNomeValue, { x: cardX + 15, y, size: 12, font, color: white });
  y -= 22;

  // Titular dos Direitos
  page.drawText('Titular dos Direitos', { x: cardX + 15, y, size: 10, font: fontBold, color: white });
  ...
  page.drawText(titularNomeValue, { x: cardX + 15, y, size: 12, font, color: white });
  y -= 22;

  // Declaração
  page.drawText('Declaração', { x: cardX + 15, y, size: 10, font: fontBold, color: white });
  y -= 14;
  ...
  const decMaxW = cardW - 30; const decMinY = cardY - cardH + 15;
  let afterDec = drawJustifiedParagraph(page, declaracao, cardX + 15, y, decMaxW, decMinY, font, 10, white, 2);
  y = afterDec.y - 6;
  ...
  const verifyUrl = hash ? `https://www.cyberregistro.com.br/verify?mlb=${encodeURIComponent(input.mlbCode)}&ts=${encodeURIComponent(ts)}&h=${hash}` : (input.permalink || 'https://www.cyberregistro.com.br');
  ...
  if (qr) {
    ...
    // Label removido abaixo do QR
  }

  // Seção de Hash Criptográfica - Azul escuro com borda cyan
  const hashBoxX = M;
  ...
  // Ícone de cadeado cyan ao lado do título
  ...
  // 1. TIMESTAMP
  page.drawText('TIMESTAMP:', { x: M, y: footerY, size: 7, font: fontBold, color: white });
  page.drawText(timestampFormatted, { x: M + 52, y: footerY, size: 7, font, color: white });
  footerY -= 12;

  // 2. TÍTULO DA OBRA
  page.drawText('TÍTULO DA OBRA:', { x: M, y: footerY, size: 7, font: fontBold, color: white });
  page.drawText(sanitizeWinAnsi(safe(input.mlbCode)), { x: M + 75, y: footerY, size: 7, font, color: white });
  footerY -= 18;

  // 3. CYBER REGISTRO
  page.drawText(brand1, { x: M, y: footerY, size: 9, font: fontBold, color: white });
  footerY -= 12;

  // 4. CNPJ
  page.drawText(brand2, { x: M, y: footerY, size: 7, font, color: white });
